...
namespace DaprAspire.IdentityService.Infrastructure.Identity.Stores
{
        ...
        where TRole : ApplicationRole
    {
        ...
        public RoleStore(IMongoDatabase mongoDatabase, IdentityErrorDescriber describer)
        {
            var collName = typeof(TRole).Name;
            _collection = mongoDatabase.GetCollection<TRole>(collName);
        ...
        public async Task<IList<Claim>> GetClaimsAsync(TRole role, CancellationToken cancellationToken = default)
        {
            ...
            return dbRole?.RoleClaims.Select(e => new Claim(e.ClaimType, e.ClaimValue)).ToList() ?? new List<Claim>();
        }
        ...
        public async Task AddClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default)
        {
            ...
            var identityRoleClaim = new ApplicationRoleClaim()
            {
                ClaimType = claim.Type,
                ClaimValue = claim.Value
            };

            role.RoleClaims.Add(identityRoleClaim);
        ...
        public Task RemoveClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default)
        {
            ...
            //keep all claims except for the claim being removed
            role.RoleClaims = role.RoleClaims.Where(x => x.ClaimType != claim.Type && x.ClaimValue != claim.Value)
                                             .ToList();
