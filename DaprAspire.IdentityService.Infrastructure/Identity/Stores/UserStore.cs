...
namespace DaprAspire.IdentityService.Infrastructure.Identity.Stores
{
        ...
        where TRole : ApplicationRole
    {
        ...
        public UserStore(IMongoDatabase mongoDatabase, IdentityErrorDescriber describer)
        {
            var userCollName = typeof(TUser).Name;
            _userCollection = mongoDatabase.GetCollection<TUser>(userCollName);

            var roleCollName = typeof(TRole).Name;
            _roleCollection = mongoDatabase.GetCollection<TRole>(roleCollName);
        ...
        public async Task SetTokenAsync(TUser user, string loginProvider, string name, string value, CancellationToken cancellationToken = default)
        {
            ...
            if (token == null)
            {
                user.Tokens.Add(new ApplicationUserToken()
                {
        ...
        public Task<TUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken = default)
        {
            ...
            return _userCollection.FirstOrDefaultAsync(x => x.NormalizedUserName == normalizedUserName, cancellationToken);
        }
        ...
        public Task AddClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default)
        {
            ...
            {
                var identityClaim = new ApplicationUserClaim()
                {
        ...
        public Task AddLoginAsync(TUser user, UserLoginInfo login, CancellationToken cancellationToken = default)
        {
            ...
            var iul = new ApplicationUserLogin
            {
                UserId = user.Id,
        ...
        public async Task AddToRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            ApplicationRole roleEntity = await FindRoleAsync(normalizedRoleName, cancellationToken);
            ...
            user.UserRoles.Add(new ApplicationUserRole { RoleId = roleEntity.Id, UserId = user.Id });
        }
        ...
        public async Task RemoveFromRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            if (roleEntity != null)
            {
                user.UserRoles.RemoveAll(x => x.RoleId == roleEntity.Id);
            }
        }
        ...
        public async Task<IList<TUser>> GetUsersInRoleAsync(string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            var filter = Builders<TUser>.Filter.AnyEq(x => x.UserRoles, new ApplicationUserRole { RoleId = role.Id });
        ...
        public async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default)
        {
            ...
            {
                var dbRole = await _roleCollection.FirstOrDefaultAsync(r => r.Id.Equals(item.RoleId), cancellationToken).ConfigureAwait(true);
        ...
        public async Task<bool> IsInRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            return dbUser?.UserRoles.Select(x => x.RoleId).Any(x => x == role.Id) ?? false;
        }
        ...
        private async Task<TRole> FindRoleAsync(string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            var role = await _roleCollection.FirstOrDefaultAsync(x => x.NormalizedName == normalizedRoleName, cancellationToken);
            return role;
        }

        private async Task<ApplicationUserToken> FindTokenAsync(TUser user, string loginProvider, string name, CancellationToken cancellationToken = default)
        {
            var dbUser = await ByIdAsync(user.Id, cancellationToken).ConfigureAwait(true);
        ...
        private Task<TUser> ByIdAsync(string id, CancellationToken cancellationToken = default)
        {
